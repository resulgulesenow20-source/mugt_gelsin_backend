...
void main() async {
  ...
  if (dukkanlar['documents'] != null) {
    for (var d in dukkanlar['documents']) {
      ...
      final address = fields['address']?['stringValue'] ?? '';
      final phone = fields['phone']?['stringValue'] ?? '';
      final mugutId = fields['mugut_id']?['stringValue'] ?? '';
      
      if (name.toLowerCase().contains('başkanabat') || address.toLowerCase().contains('başkanabat') || address.toLowerCase().contains('balkanabat')) {
        print('FOUND: $name - Address: $address, phone: $phone, mugut_id: $mugutId');
      }
    }
  }
}
